        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-2 gap-4 relative">
            ...
            <div className="space-y-2 pt-4">
              <div className="text-[10px] uppercase tracking-wider text-emerald-400/70 text-center font-medium">
                Simulation Loop
              </div>
              <Node
                icon={FileCode2}
                title="Case Generation Agent"
                subtitle="LLM → assembly"
              ...
              {overallCoverage < coverageGoal && (
                <>
                  ...
                  <Node
                    icon={Lightbulb}
                    title="Missing Case Suggestion"
                    subtitle="LLM → new scenarios"
            ...
            <div className="space-y-2 pt-4">
              <div className="text-[10px] uppercase tracking-wider text-fuchsia-400/70 text-center font-medium">
                Formal Verification
              </div>
              <Node
                icon={FileCode2}
                title="Property Generation Agent"
                subtitle="LLM → formal props"
                active={formalActive}
                accent="fuchsia"
                compact
              />
              ...
              <Node
                icon={ShieldCheck}
                title="SymbiYosys-style Checker"
                subtitle="property-based"
                active={formalActive}
                accent="fuchsia"
                compact
              />
...
function Node({
  ...
  subtitle: string;
  active: boolean;
  accent: 'emerald' | 'cyan' | 'amber' | 'fuchsia' | 'slate';
  compact?: boolean;
}) {
  const colors: Record<string, { border: string; bg: string; text: string; icon: string }> = {
    emerald: { border: 'border-emerald-500/40', bg: 'bg-emerald-500/5', text: 'text-emerald-400', icon: 'text-emerald-400' },
    cyan: { border: 'border-cyan-500/40', bg: 'bg-cyan-500/5', text: 'text-cyan-400', icon: 'text-cyan-400' },
    amber: { border: 'border-amber-500/40', bg: 'bg-amber-500/5', text: 'text-amber-400', icon: 'text-amber-400' },
    fuchsia: { border: 'border-fuchsia-500/40', bg: 'bg-fuchsia-500/5', text: 'text-fuchsia-400', icon: 'text-fuchsia-400' },
    slate: { border: 'border-slate-500/40', bg: 'bg-slate-500/5', text: 'text-slate-300', icon: 'text-slate-400' },
  };
  const c = colors[accent];
  return (
    <div
      className={`rounded-md border px-2 ${compact ? 'py-1.5' : 'py-2'} transition-all ${c.border} ${c.bg} ${active ? `ring-1 ring-offset-0 ${c.border} shadow-lg` : 'opacity-90'}`}
    >
      <div className="flex items-center gap-2">
        <Icon className={`h-3.5 w-3.5 flex-shrink-0 ${active ? c.icon : c.icon + '/60'}`} />
        <div className="flex-1 min-w-0">
  ...
  return (
    <div className="flex justify-center">
      <svg width="14" height="14" viewBox="0 0 14 14">
        <line x1="7" y1="0" x2="7" y2="9" stroke={active ? 'oklch(0.55 0.1 155)' : 'oklch(0.4 0.014 240)'} strokeWidth="1" strokeDasharray={active ? '' : '2 2'} />
        <polygon points="3,9 11,9 7,13" fill={active ? 'oklch(0.55 0.1 155)' : 'oklch(0.4 0.014 240)'} />
      </svg>
    </div>
  );
}
