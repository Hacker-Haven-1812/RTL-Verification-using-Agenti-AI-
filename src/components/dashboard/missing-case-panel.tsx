        ...
        <ScrollArea className="h-full max-h-[280px]">
          <div className="px-3 pb-3 space-y-2 pt-2">
            {suggestions.length === 0 ? (
              ...
              suggestions.map((s, i) => (
                <div key={i} className="fade-in rounded-md border border-primary/30 bg-primary/5 p-2">
                  <div className="flex items-start gap-2">
                    <div className="flex-shrink-0 w-5 h-5 rounded-md bg-primary/10 border border-primary/30 flex items-center justify-center text-[10px] font-bold text-primary">
                      {i + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium text-primary/80">{s.scenario}</div>
                      ...
                      {s.suggestedInstructions.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {s.suggestedInstructions.map((inst, j) => (
                            <Badge key={j} variant="outline" className="text-[9px] py-0 px-1 font-mono border-primary/30 text-primary/80 bg-primary/5">
                              {inst}
                            </Badge>
                          ))}
                        </div>
